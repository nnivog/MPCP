...
# 1. Add Locations nav tab if not present
if "switchTab('locations')" not in content:
    ...
    # fallback for emoji version
    content = content.replace(
        "onclick=\"switchTab('reports')\">📈 Reports</button>",
        "onclick=\"switchTab('reports')\">📈 Reports</button>\n    <button class=\"nav-tab\" onclick=\"switchTab('locations')\">📍 Locations</button>"
    )

# 2. Add tab panel div if not present
if 'id="tab-locations"' not in content:
    content = content.replace(
        '<div id="tab-reports" class="tab-panel">',
        '<div id="tab-locations" class="tab-panel"><div class="spinner"></div></div>\n\n<div id="tab-reports" class="tab-panel">'
    )

# 3. Add locations array to state if not present
if 'locations:[]' not in content and "locations: []" not in content:
    content = content.replace('const S = {', 'const S = {\n  locations:[],')

# 4. Load locations in loadAll if not present
if '/api/locations' not in content:
    content = content.replace(
...
# 5. Add renderLocations to router if not present
if "renderLocations" not in content:
    content = content.replace(
...
function renderLocations() {
  ...
  var html = '<div class="section-head"><div><h2>Locations</h2><p>' + S.locations.length + ' location(s) configured</p></div>' +
  ...
  if (!S.locations.length) {
    ...
    q('tab-locations').innerHTML = html;
    return;
  }
  ...
  S.locations.forEach(function(loc) {
    ...
    html +=
      '<div class="card" style="border-top:3px solid ' + color + ';padding:16px;opacity:' + opacity + '">' +
...
function openLocForm(id) {
  ...
  var typeOpts = TYPES.map(function(t){ return '<option value="' + t + '"' + (loc && loc.type===t ? ' selected' : '') + '>' + t + '</option>'; }).join('');
  var deptOpts = DEPTS.map(function(d){ return '<option value="' + d + '"' + (loc && loc.dept===d ? ' selected' : '') + '>' + d + '</option>'; }).join('');
  ...
  openModal(loc ? 'Edit Location' : 'Add Location', body, [{
    ...
    cls: 'btn-primary',
    fn: function() {
      var code = q('lf-code').value.trim();
      var name = q('lf-name').value.trim();
      if (!code || !name) { toast('Code and name are required', 'error'); return; }
      var d = {
        code:    code,
        name:    name,
        address: q('lf-address').value.trim(),
        type:    q('lf-type').value,
        dept:    q('lf-dept').value,
        active:  q('lf-active').value === '1',
...
content = content.replace('</script>', loc_js + '\n</script>')
